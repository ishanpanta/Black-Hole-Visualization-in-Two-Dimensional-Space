...
#  We first write the vertex shader to be able to receive position and colors for each vertex.
# deals with vertices
vertexShader = """
...
def blackHoleAnimation(value):
    ...
    for i, x in enumerate(tempVertices):
        # distance between photon and blackhole center.
        ...
        if dist != 0.0:
            # the force of attraction between the photon and the black hole
            factor = adjustedG * adjustedMass / (dist * dist)
            factor = factor
            # adjust the position of photon according to the force it experience at its current position.
...
#  This function requests a buffer to the GPU and then passes buffer data from the CPU to the GPU.
def init():
    ...
    program = createProgram(
        #  This function is used to compile the vertex and fragment shader
    ...
    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, vertexBuffer)
    # needs to gpu how many bytes willl be sent.
    # 
    gl.glBufferData(
        gl.GL_ARRAY_BUFFER,
        accretionDisk.nbytes
    ...
    #  yesle chai yaha xa vanera vandinxa
...
# the render function clears the screen and draws the objects using the Gl_TRIANGLES primitive
def render():
    global lightRay
    gl.glClear(gl.GL_COLOR_BUFFER_BIT)
    gl.glClearColor(0.0, 0.0, 0.0, 0.0)
    gl.glPointSize(3)
    # vao draw han vanera vanxa
